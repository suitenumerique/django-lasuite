...
4. **Resource Server Client**: A view fetches the user's access token from the
   session and uses it to call `/external_api/v1.0/users/me/` on an external resource
   server.
...
3. After authentication, click **Call Resource Server /external_api/v1.0/users/me/**
...
def resource_server_me(request):
    ...
    response = requests.get(
        f"{settings.RESOURCE_SERVER_BASE_URL}/external_api/v1.0/users/me/",
